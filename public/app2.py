...
df_grund = pd.DataFrame(
    dict(a=a, b=b, c=c, d=d , e=e ,f=f, g=g , z=z , zz=zz  ,menge=menge)
)

gesamt_grund = px.sunburst(df_grund, path=['z', 'b', 'c', 'g','d','e','f'], values="menge",
...
df = pd.DataFrame(
    dict(a=a, b=b, c=c, d=d , e=e ,f=f, g=g, zz=zz ,menge=menge)
)

bu_grund = px.sunburst(df, path=['zz' , 'a', 'b', 'c', 'd', 'e','f'], values="menge",
hover_name="menge",
#hover_data={'all':False},
maxdepth=2
#template="ggplot",
...
with middle_column:
    st.subheader("Übersicht Symptome und Gründe")
    st.plotly_chart(gesamt_grund)
with right_column:
    st.subheader("Übersicht Aufteilung BUs")
